  ...
  while ((match = streamRegex.exec(pdfText)) !== null) {
    ...
    if (hasFlate) {
      try {
        ...
        // Fallback to raw text if inflate fails on malformed streams.
...
function detectCompanyName(lines) {
  const priorityPatterns = [
    /^(banque|bank|credit|crédit|soci[eé]t[eé])/i,
    /(sa|sarl|sas|spa)\b/i,
  ];
...
function splitColumns(line) {
  ...
  // Detect date + libellé + montant style lines
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/api/extract') {
    try {
      ...
      const pdfBuffer = Buffer.from(body.base64Pdf, 'base64');
